...
def decision_tree(X, Y, scale, level=0):
    ...
    for i in range(n):
        ...
        if scale[i] == CATEGORICAL:
            info_s = 0
            info_s = np.sum(np.abs(np.average(Y)) / np.abs(np.average(Y)) * info)
            gain.append(info - info_s)

            #вычислить информационный выигрыш от разбиения исходного множества
...
def Info(set):
    ...
    entropia = -sum((np.log2(i / m) * i / m) for i in np.unique(set))
    info = entropia
